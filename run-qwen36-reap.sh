...
if [[ "$SERVE" == "1" ]]; then
  echo "SERVE=1 URL=http://$HOST:$PORT/v1 MODEL_ALIAS=$ALIAS PARALLEL=$PARALLEL MTP=$MTP REASONING=$REASONING SPEC_TYPE=$SPEC_TYPE SLOT_SAVE_PATH=${SLOT_SAVE_PATH:-<disabled>} CACHE_RAM=${CACHE_RAM:-<default>} CACHE_REUSE=$CACHE_REUSE"
else
